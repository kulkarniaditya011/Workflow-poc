...
public ApiResponse<List<TenantDTO>> getAllTenants() {
    ...
    List<TenantDTO> requestTenantDTOS = pagebleObject.mapList(tenants, TenantDTO.class);
...
public ApiResponse<List<TenantDTO>> getAllTenants() {
    ...
    List<TenantDTO> requestTenantDTOS = pagebleObject.mapList(tenants, TenantDTO.class);
    return ResponseUtil.getResponse(requestTenantDTOS, "Tenant details");
}
```
...
public ApiResponse<String> createTenant(TenantDTO requestTenantDTO) {
    ...
    if (tenantRepository.findByTenantId(requestTenantDTO.getTenantId()).isPresent()) {
        throw new RestApiException(
            "Tenant with ID " + requestTenantDTO.getTenantId() + " already exists",
            HttpStatus.BAD_REQUEST);
    }
